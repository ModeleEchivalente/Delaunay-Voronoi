    ...
    while (window.isOpen())
    {
        ...
        while (window.pollEvent(event))
        {
             ...
             case sf::Event::KeyReleased:
                  ...
                  else if (event.key.code == sf::Keyboard::T) hideDelaunay = !hideDelaunay;
                  break;
        ...
        for (int i = 0; i < VD.Edges.size() && !hideVoronoi; i++) {
             window.draw(voronoiLines[i], 2, sf::Lines);
        }

        for (int i = 0; i < VD.Delaunay.size() && !hideDelaunay; i++) {
